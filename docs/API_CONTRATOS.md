...
- Sin endpoints nuevos: el usuario elige periodo de pago (**bimestral** o **mensual**) y captura un monto aproximado en MXN para ese periodo (cada valor se conserva de forma independiente), además de segmento (residencial / comercial / industrial) y contexto de tarifa CFE; se arma texto con `buildSolarInquiryMessage` y se abre `wa.me` con el mismo criterio que el formulario.
