...
- **`GeneratorQuickQuote` — recomendación más explícita**: en resultado se muestra el mensaje “Con lo que seleccionaste, el generador recomendado inicialmente es…” junto al modelo/línea, explicación corta y CTA de WhatsApp con resumen del escenario capturado.
- **Catálogo Generadores (`Equipos liquid cooled y comerciales`)**: la imagen de esa línea cambia a `liquid-nuevo.png` (antes `liquid.png`) para mostrar el visual actualizado del bloque.
- **`GeneratorQuickQuote` (`/generadores`) — recomendación explícita de línea/equipo**: la tarjeta de resultado ahora muestra claramente la capacidad estimada, el generador recomendado (por modelo/línea), badge de segmento, explicación breve del porqué, nota de estimación inicial y CTA para enviar la selección por WhatsApp con resumen precargado.
...
- **SolarInquiryPanel (periodo de pago)**: el control de monto se divide en **Bimestre** y **Mensual** con selector explícito; cada periodo conserva su propio valor (independientes), rango, accesos rápidos y captura exacta. En la cabecera de monto se muestra solo el periodo activo (si eliges bimestre, solo bimestre; si eliges mensual, solo mensual). El mensaje de WhatsApp usa el periodo activo.
- **Solar (`#solar`, home)**: se elimina el kicker en mayúsculas «Paneles solares» encima del h2; la sección abre con el titular *Paneles solares a la medida de lo que realmente consumes* y se reenumeran los `data-stagger` del bloque.
- **SolarInquiryPanel**: el titular del panel pasa a **Tres datos y tendrás una propuesta lista** (sustituye la formulación anterior con pago/segmento/tarifa). Se quita el párrafo aclaratorio bajo ese titular (primer contacto / cotización numérica / recibos CFE); también se elimina la etiqueta superior **Contexto para tu consulta**. Se retiran el texto *Rango sugerido* (min–max) y la fila de montos bajo el control de costo ($200 … $120,000); el slider y los accesos rápidos por monto siguen acotados en código (`COST_MIN` / `COST_MAX`).
- **Paneles solares (copy garantía producción)**: en la sección `#solar`, en la tarjeta de línea solar (home) y en la ficha «Garantía y producción» de `solarSystemItems`, el mensaje pasa a **25 a 30 años de garantía en producción de energía** (se mantiene el alcance de pieza/instalación y la referencia a fabricante/condiciones donde aplica).
- **Solar (sección `#solar`)**: el botón **Solicitar evaluación técnica** va justo debajo del panel de consulta (`SolarInquiryPanel`) y queda **centrado** en el contenedor `max-w-2xl`; la rejilla `solarSystemItems` sigue debajo.
